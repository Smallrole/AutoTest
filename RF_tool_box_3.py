        ...
        if operation == 'remove':
            if target == '*':
                for file_name in all_file:
                    ...
                    print('删除文件，文件路径为： ' + file_path + '\\' + file_name)
    ...
    def write_test_case_file(self, open_test_case_file, mode, case_content, file_type='', *test_case_data):
        if mode == 'only_write':
            ...
            if file_type == 'interface':
                ...
                suite_name = test_case_data[0] + '-' + test_case_data[3]
                suite_documentation = test_case_data[27]
                ...
                for t in test_case_data[3].split(';'):
                    case_tags = case_tags + '    ' + t
                case_content = Template(case_content)
                case_content = case_content.substitute(function_case_name=(test_case_data[0] + '-' + test_case_data[2]),
                                                       test_case_tags=case_tags,test_case_id=test_case_data[0])
                open_test_case_file.writelines(case_content)
                self.all_test_case_sum += 1
        return open_test_case_file
